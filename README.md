...
line spacing, a dark theme and a view scale ratio. File names that begin with
a dot will not be shown if the hidden option is enabled.  The red underline
under misspelled English words is controlled by the spell check setting
(currently, no suggestions are shown for misspelled words on desktop
platforms). The tight line spacing option allows more lines to fit on the
screen - it is recommended when using a mouse, but may not leave enough space
for touch interfaces.  A darker theme can be enabled in place of the default
light theme. The view scale ratio can be set to make the content smaller or
larger (useful for high-dpi displays).

On desktop platforms, there are also options to set the working directory,
remember window geometry and hide the window title bar. The working directory
is where all files in the main file list are stored.  The window size and
position will be restored from the previous session if enabled.  The window
title bar can be hidden (sometimes useful under Linux/Wayland).
